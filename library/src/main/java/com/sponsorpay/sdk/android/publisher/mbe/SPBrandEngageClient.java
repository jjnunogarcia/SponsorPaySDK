...
import com.sponsorpay.sdk.android.publisher.mbe.mediation.*;
...
public class SPBrandEngageClient {
   ...
   * possible.
   *
   * @param credentials The credentials that will be used for this query.
   ...
   *
   * @param activity             The activity that will be used to show the engagement
   * @param playThroughMediation A boolean informing if the offer is a mediated one (see {@link #playThroughMediation(ValueCallback)})
  ...
  private void changeStatus(String status) {
    if (status.equals(SP_REQUEST_STATUS_PARAMETER_STARTED_VALUE)) {
      ...
      if (setClientStatus(SPBrandEngageOffersStatus.SHOWING_OFFERS)) {
      ...
      // Prevent showing dialog when sth happens with the mediation JS
      mHandler.removeMessages(VIDEO_EVENT);
      clearWebViewPage();
   ...
   *
   * @param currencyName The currency name that will override the default one
   ...
   *
   * @param currencyId The currency ID that will override the default one
   * @return true if successful in setting the name, false otherwise
   */
   ...
   *
   * @param parameters The additional parameters map
   ...
   *
   * @param listener The listener called after a successful request at SponsorPay's
   ...
   *
   * @param listener The {@link SPBrandEngageClientStatusListener}
   * @return true if successful in setting the listener, false otherwise
   */
  ...
  public void playThroughMediation(ValueCallback<Boolean> valueCallback) {
    mJSInterface.playThroughThirdParty(mWebView, valueCallback);
  }
  ...
  private boolean setClientStatus(SPBrandEngageOffersStatus newStatus) {
    if (mStatus != newStatus) {
      int diff = newStatus.ordinal() - mStatus.ordinal();
      // prevent the wrong state to be set
      // equals to 0 is prevented in the if above
      if (diff <= 1) {
        mStatus = newStatus;
